...
def t100_dept_report_list(request, dept_id):
    is_grpxxx=request.user.groups.filter(name='grp005').exists()
    if not is_grpxxx:
       return redirect('/admin/login/?next=/projectnote/t100_dept_report')
    current_user=request.user.username
    # item_list = Sop.objects.filter(is_active = True).order_by('code')[:500]
    # TO SHOW ALL , INCLUDING ACTIVE OR NOT
    item_list = TrackReport00.objects.filter(deptId = dept_id).order_by('dept','a')[:1500]

    context = {'item_list': item_list,'current_user':current_user,'dept_id':dept_id}
    return render(request, 'projectnote/t100_dept_report_list.html', context)
...
def t100_dept_report_list2(request, dept_id,status):
    ...
    # item_list = Sop.objects.filter(is_active = True).order_by('code')[:500]
    # TO SHOW ALL , INCLUDING ACTIVE OR NOT
    item_list = TrackReport00.objects.filter(deptId = dept_id).filter(h = status).order_by('dept','a')[:1500]

    context = {'item_list': item_list,'current_user':current_user,'dept_id':dept_id,'status':status}
    return render(request, 'projectnote/t100_dept_report_list2.html', context)
...
def t100_dept_report_list_status(request, dept_id,status):
    ...
    # item_list = Sop.objects.filter(is_active = True).order_by('code')[:500]
    # TO SHOW ALL , INCLUDING ACTIVE OR NOT
    item_list = TrackReport00.objects.filter(deptId = dept_id).filter(status = status).order_by('dept','a')[:1500]

    context = {'item_list': item_list,'current_user':current_user,'dept_id':dept_id,'status':status}
    return render(request, 'projectnote/t100_dept_report_list_status.html', context)
...
def t100_report_sum(request):
    ...
    if not is_grpxxx:
    #    return redirect('/projectnote')
       return redirect('/admin/login/?next=/projectnote/t100_report_sum')
    current_user=request.user.username
    # item_list = Sop.objects.filter(is_active = True).order_by('code')[:500]
    # TO SHOW ALL , INCLUDING ACTIVE OR NOT
...
def t100_report_sum2(request):
    ...
    # item_list = Sop.objects.filter(is_active = True).order_by('code')[:500]
    # TO SHOW ALL , INCLUDING ACTIVE OR NOT
    item_list = TrackReport00.objects.values("dept","status","h").annotate(Count('a'))
    # subtotal =Receiving.objects.values("").annotate(Count('FG')).
    context = {'item_list': item_list,'current_user':current_user}
...
def t100_report_sum_dept_id_status(request,dept_id,status):
    ...
    # item_list = Sop.objects.filter(is_active = True).order_by('code')[:500]
    # TO SHOW ALL , INCLUDING ACTIVE OR NOT
    item_list = TrackReport00.objects.filter(deptId=dept_id).filter(h=status)
...
def t100_dept_report(request):
    ...
    item_list2 = T100Dept.objects.filter(D0__gt = 0).aggregate(sumd0=Sum('D0'),sumd1=Sum('D1'),sumd2=Sum('D2'),sumd3=Sum('D3'),sumd4=Sum('D4'),sumd5=Sum('D5'),sumd6=Sum('D6'),sumd7=Sum('D7'),sumd8=Sum('D8'))
